            ...
            for (int i = 0; i < tbstyle.Rows.Count; i++)
            {
                ...
                if (row["display"].ToString() == "1")
                {
                    ...
                    if (tbdetail.Columns.Contains(colname) == true)
                    {
                        ...
                        {
                            Helper.Barcode.CreateBarcodeImage(g, rec.X, rec.Y, "C", dr[colname].ToString(), 1F, rec.Height);
                        }
                        else if (format == "二维码")
                        {
                            Helper.QRCode code = new Helper.QRCode();
                            Bitmap bit = code.grant_qrcode(dr[colname].ToString(), rec.Width, rec.Width);
                            g.DrawImage(bit, rec);
                        }
                        else
                        {
