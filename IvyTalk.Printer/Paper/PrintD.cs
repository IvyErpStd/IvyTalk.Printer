...
using IvyTalk.Printer.Paper.Controls;

namespace IvyTalk.Printer.Paper
{
    public class PrintD : Form, IBLL.IPrint, IDesign
    {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                else if (TbtnSum.Checked == true)
                {
                    ...
                    if (tbdetail != null)
                    {
                        ...
                        {
                            var con = new PrintObject3();
                            IPrintObject ins = con;
                            ins.SetSelectControl(this);
                            ISizeable sizeable = (ISizeable)ins;
                            sizeable.Location = p;
                            con.Field = def;
        ...
        private void contextMenuStrip1_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            try
            {
                ...
                else if (e.ClickedItem.Text == "粘贴")
                {
                    ...
                    int x2 = -9999;
                    int y2 = -9999;
                    foreach (IPrintObject ins in lstcopy)
                    {
                    ...
                    if (flag == 1)
                    {
                        List<IPrintObject> lsttemp = new List<IPrintObject>();
                        foreach (IPrintObject ins in SelectObjects)
                        {
                            lsttemp.Add(ins);
                        }
                        foreach (IPrintObject ins in lsttemp)
                        {
                            ins.Selected = false;
                        }
                        SelectObjects.Clear();
        ...
        List<IPrintObject> IDesign.GetSelectObjects()
        {
        ...
        private void pnl_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                SelectObjects = new List<IPrintObject>();
        ...
        List<IPrintObject> lstcopy = new List<IPrintObject>();
        ...
        string IDesign.xml
        {
            get
            {
                ...
                foreach (IPrintObject ins in pnl.Controls)
                {
                    IChangeAreaAble area = (IChangeAreaAble)ins;
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (toolStripComboBox1.Focused == false)//字号
            {
                ...
                foreach (IPrintObject ins in SelectObjects)
                {
                    if (ins.GetType().GetInterface(typeof(IFontable).ToString()) != null)
                    {
                        IFontable fontable = (IFontable)ins;
                        if (size == 0)
                        {
            ...
            if (1 == 1)
            {
                ...
                foreach (IPrintObject ins in SelectObjects)
                {
                    if (ins.GetType().GetInterface(typeof(IContextAlignAble).ToString()) != null)
                    {
                        IContextAlignAble item = (IContextAlignAble)ins;
                        if (align == 0)
                        {
                            align = item.Align;
                        }
        ...
        private void toolStripSplitButton1_ButtonClick(object sender, EventArgs e)
        {
            ...
            foreach (IPrintObject ins in SelectObjects)
            {
                if (ins.GetType().GetInterface(typeof(IBorderable).ToString()) != null)
                {
                    flag = 1;
                    IBorderable item = (IBorderable)ins;
        ...
        private void toolStripSplitButton3_ButtonClick(object sender, EventArgs e)
        {
            ...
            foreach (IPrintObject ins in SelectObjects)
            {
                if (ins.GetType().GetInterface(typeof(IColorable).ToString()) != null)
                {
                    IColorable item = (IColorable)ins;
