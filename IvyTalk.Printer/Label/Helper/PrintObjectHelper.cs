...
namespace IvyTalk.Printer.Label.Helper
{
    ...
    {
        public static Type[] GetTypes(IPrintObject[] objs)
        {
            List<Type> lst = new List<Type>();
         
          
            lst.Add(typeof(IContextable));
            lst.Add(typeof(IContextAlignAble));
            lst.Add(typeof(IFontable));
            lst.Add(typeof(IColorable));
            lst.Add(typeof(IBorderable));
            lst.Add(typeof(IFormatable));
            lst.Add(typeof(IImageAble));
            lst.Add(typeof(IFieldAble));
            lst.Add(typeof(IChangeAreaAble));
            lst.Add(typeof(ISizeable));
            lst.Add(typeof(IDeleteable));
            lst.Add(typeof(IGridable));
            lst.Add(typeof(IPrintObject));
            Dictionary<Type, int> dic = new Dictionary<Type, int>();
            foreach (IPrintObject obj in objs)
            {
        ...
        {
            if (rec2.Contains(new Point(rec.X, rec.Y)) == true
                || rec2.Contains(new Point(rec.X, rec.Y + rec.Height)) == true
             || rec2.Contains(new Point(rec.X + rec.Width, rec.Y)) == true
                || rec2.Contains(new Point(rec.X + rec.Width, rec.Y + rec.Height)) == true)
            {
                return true;
            }
            else
